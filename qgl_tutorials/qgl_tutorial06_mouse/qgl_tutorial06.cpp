...
void Tutorial06::initializeGL() {
    ...
    simpleUniformId = glGetUniformLocation(programID, "myTextureSampler");
    ...
    mTexture = new QOpenGLTexture( QImage(":/res/uvtemplate.bmp").mirrored() );
...
void Tutorial06::paintGL(void) {
    ...
    glVertexAttribPointer(
        positionAttr,  // The attribute we want to configure
        3,                            // size
        GL_FLOAT,                     // type
        GL_FALSE,                     // normalized?
        0,                            // stride
...
void Tutorial06::resizeGL(int w, int h) {
    ...
    mProjectionMatrix.perspective(45.0, ((float)w / h), 0.1, 100.0);
}
...
void Tutorial06::mousePressEvent(QMouseEvent *e) {
    ...
    qDebug("mousePressEvent, x:%d, y:%d", oldX, oldY);
}
